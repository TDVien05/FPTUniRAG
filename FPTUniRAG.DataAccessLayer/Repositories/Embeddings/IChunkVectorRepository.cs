...
public interface IChunkVectorRepository
{
    ...
    Task<IReadOnlyList<ChunkSimilarityRecord>> SearchSimilarChunksAsync(string tableName, Guid subjectId, string model, float[] queryEmbedding, int limit, CancellationToken cancellationToken = default);
...
public sealed record ChunkSimilarityRecord(Guid ChunkId, string Content, int ChunkIndex, Guid DocumentId,
    string DocumentTitle, string SubjectCode, string SubjectName, string ChapterTitle, double SimilarityScore);
