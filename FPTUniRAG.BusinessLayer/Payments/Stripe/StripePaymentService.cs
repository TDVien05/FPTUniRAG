...
namespace FPTUniRAG.BusinessLayer.Payments.Stripe;
...
public sealed class StripePaymentService : IStripePaymentService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var plan = await _paymentRepository.GetActivePlanAsync(normalizedPlanCode, cancellationToken);
        ...
        if (!IsValidStripePriceId(plan.StripePriceId))
        {
            ...
            await _paymentRepository.SavePlanAsync(plan, cancellationToken);
        }
        ...
        await _paymentRepository.AddStripeTransactionAsync(transaction, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var transaction = await _paymentRepository.FindStripeTransactionAsync(normalizedSessionId, cancellationToken);
        ...
        if (!response.IsSuccessStatusCode)
        {
            await _paymentRepository.SaveStripeTransactionAsync(transaction, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var activation = await _paymentRepository.GetStripeActivationAsync(transaction.UserId, cancellationToken);
        var currentSubscription = activation.CurrentSubscription;
        ...
        if (currentSubscription is not null)
        {
            ...
            {
                previousStripeSubscriptionId = ExtractStripeSubscriptionId(activation.PreviousRawResponseJson);
            }
        ...
        await _paymentRepository.ActivateStripeSubscriptionAsync(
            transaction, currentSubscription, checkoutId, stripeSubscriptionId, now, cancellationToken);
    }
