...
public sealed class AnalysisService(IAdminReportingRepository repository, ISubscriptionPlanManagementService planManagementService) : IAnalysisService
{
    public async Task<AnalysisDashboardDto> GetAnalysisAsync(string? requested, CancellationToken cancellationToken = default)
    {
        ...
        var planOverview = await planManagementService.GetPlansAsync(cancellationToken);
        var purchases = data.PaidTransactions.Select(p => new PlanPurchaseDto(p.StudentName, p.StudentEmail, p.PlanName, p.Amount, p.PurchasedAt)).ToList();
        ...
        return new(range.Period, range.Label, data.ActiveSubscriptions.Select(x => x.UserId).Distinct().Count(), data.PaidTransactions.Count, data.PaidTransactions.Sum(x => x.Amount), purchases, trend, planOverview);
    }
